...
def kv_to_argv(d_global: dict | None, d_step: dict | None) -> List[str]:
    """
    Vytvoří argv pro argparse: ['--key','val', ...]
    Precedence: hodnoty ze sekce 'global', potom ze sekce daného kroku (ty přepíšou global).
    Skutečné CLI argumenty vždy vítězí (přijdou až při volání modulu).
    """
    argv: List[str] = []
