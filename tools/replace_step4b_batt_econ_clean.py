...
def main():
    if not TARGET.exists():
        raise SystemExit(f"Soubor neexistuje: {TARGET}")

    backup = TARGET.with_suffix(".py.broken")
    shutil.copy2(TARGET, backup)
    TARGET.write_text(NEW_CONTENT, encoding="utf-8")
    print(f"[OK] Přepsáno: {TARGET.relative_to(ROOT)}")
    print(f"[i] Původní verze uložena jako: {backup.name}")
