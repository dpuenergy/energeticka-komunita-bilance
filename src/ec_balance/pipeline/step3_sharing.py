...
def share_pool_degree_limited(
    ...
    *,
    ...
    """Proporční sdílení po hodinách s omezením počtu příjemců na zdroj."""
    ...
    for ts in idx:
        ...
        # cílové pokrytí a nabídka (proporční k popt./nabídce)
...
def main():
    ap = argparse.ArgumentParser(description="Krok 3 – sdílení v komunitě (pool, degree-limit)")
    ...
    ap.add_argument("--max_receivers", type=int, default=None, help="Max počet příjemců na jeden zdroj v hodině (alias).")
    ap.add_argument("--max_recipients", type=int, default=None, help="Max počet příjemců na jeden zdroj v hodině (alias).")
    ap.add_argument("--allow_self_pair", action="store_true", help="Povolit alokaci na tentýž objekt (default: NE).")
    ap.add_argument("--site_map_csv", default="", help="(Kompatibilita CLI – nevyužito zde)")
    args = ap.parse_args()
    ...
    # načti vstupy (local_self zatím nevyužíváme přímo – je jen meta)
    ...
    print(f"[OK] Sharing hotovo. Limit příjemců = {max_rec}, self_pair = {args.allow_self_pair}")
